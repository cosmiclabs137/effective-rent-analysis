    ...
    const calculateDeal = (deal) => {
        ...
        for (let period = 0; period < deal.term; period++) {
            if (period > 11 && period % 12 === 0) {
                currentRate = currentRate * (1 + deal.annualEscalations / 100);
            }
        ...
        for (let period = 0; period < deal.term; period++) {
            ...
            otherRecurringCosts[period] =
                deal.otherOneTimeTenantCost * currentRecurringCostGrowthRate;
        }
        ...
        for (let period = 0; period < deal.term; period++) {
            if (period > 11 && period % 12 === 0) {
                currentContribGrowthRate =
                    currentContribGrowthRate * (1 + deal.globalInflation / 100);
            }
        ...
        occupancyOpExCommissionsTotal.current = occupancyOpExCommissions.reduce(
            (acc, value) => acc + value,
            0
        );
        ...
        const data = rates.map((rate, period) => {
            return {
                ...
                otherMonthlyTenantCost: toCurrency(otherRecurringCosts[period]),
