...
import { calculateDeal } from "../finance";

// custom hook for calculating the deals
const useCalculateDeal = (deal, metricsDispatch, metrics) => {
    const [dealResults, setDealResults] = React.useState({});
    const [landlordResults, setLandlordResults] = React.useState({});
    const [tenantResults, setTenantResults] = React.useState({});

    React.useEffect(() => {
        const { calculatedDeal, landlordResults, tenantResults } =
            calculateDeal(deal);

        setDealResults(calculatedDeal);
        setLandlordResults(landlordResults);
        setTenantResults(tenantResults);
        // metricsDispatch({
        //     type: "updated",
        // });
        // metricsDispatch({
